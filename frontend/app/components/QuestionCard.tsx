...
function ChoiceCard({ question, isMulti, onAnswer, disabled }: Props & { isMulti: boolean }) {
  ...
  return (
    <div className="fade-up">
      ...
      {!('subtitle' in question && question.subtitle) && <div className="mb-6" />}
        ...
        {opts.map(opt => {
          ...
          return (
            <button
              ...
              className={[
                'w-full flex items-center gap-4 px-5 py-4 border-2 text-left cursor-pointer',
            ...
            >
              ...
              <span className={['font-sans text-[15px] font-medium flex-1', on ? 'text-bg' : 'text-dark'].join(' ')}>
                {opt.label}
              </span>
              {on && <span className="font-mono text-[16px] text-bg flex-shrink-0">✓</span>}
            </button>
          )
        })}
      </div>
